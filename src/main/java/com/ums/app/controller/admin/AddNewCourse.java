...
public class AddNewCourse extends BaseServlet {
    ...
    @RequiresPermission(Permission.ADD_NEW_COURSE)
    @Override
    protected void doPost(HttpServletRequest req, HttpServletResponse resp) throws ServletException, IOException {
        try (BufferedReader reader = req.getReader()) {

            Claims claims = (Claims) req.getAttribute("claims");
            Number uidNum = (Number) claims.get("uid");
            int uid = uidNum != null ? uidNum.intValue() : -1;
            ...
            if (courses.getCode() == null || courses.getCode().isEmpty()
                    || courses.getTitle() == null || courses.getTitle().isEmpty()) {
